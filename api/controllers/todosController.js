...
const todos = {
  async create({ body, decoded }, res, next) {
    try {
      ...
      const todo = await Todo.create({ title,description,deadline,priority,status,userId });
  ...
  async updateStatus({ body, decoded, params }, res, next) {
    try {
      const todo = await Todo.findOne({ where: { id: params.todoId, userId: 2 } });
      if (!todo) {
        return res.status(400).send({ error: 'Wrong todo id' });
      }
      const updatedTodo = await Todo.update({ status: body.status},
        {
          where: { id: todo.id },
          returning: true,
          plain: true
